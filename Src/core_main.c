...
#if HAS_FLOAT
	ee_printf("Total time (secs): %d\n",(long unsigned)(time_in_secs(total_time)));
	if (time_in_secs(total_time) > 0)
		ee_printf("Iterations/Sec   : %d\n",(long unsigned)(default_num_contexts*results[0].iterations/time_in_secs(total_time)));
#else 
...
#if HAS_FLOAT
		if (known_id==3) {
			ee_printf("CoreMark 1.0 : %d / %s %s",(long unsigned)(default_num_contexts*results[0].iterations/time_in_secs(total_time)),COMPILER_VERSION,COMPILER_FLAGS);
